...
import com.fouo.common.utils.StringPool;
...
public class SqlKeyword {
    ...
    public static void buildCondition(Map<String, Object> query, QueryWrapper<?> qw) {
        ...
        query.forEach((k, v) -> {
            ...
            if (k.endsWith(EQUAL)) {
                ...
                if(value.length() > 32){
                    throw new RuntimeException("查询字段不允许超过32字数");
                }
                if(v instanceof String){
                    if(value !=null && value.length()>0){
                        if(checkOne(value)){
                            qw.like(getColumn(k, LIKE), "["+value+"]");
                        }else {
                            qw.like(getColumn(k, LIKE), value);}
                    }
                }
    ...
    private static boolean checkOne(String str){
