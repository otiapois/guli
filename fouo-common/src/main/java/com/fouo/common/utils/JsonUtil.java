...
import com.fouo.common.exception.Exceptions;
...
public class JsonUtil {
    ...
    public static Map<String, Object> toMap(String content) {
        try {
            Map map= getInstance().readValue(content, Map.class);
            return map;
        } catch (IOException e) {
            log.error(e.getMessage(), e);
        }
    ...
    public static <T> T readValue(String json, Class<?> parametrized, Class<?> parametersFor,
        ...
        if (parameterClasses == null || parameterClasses.length == 0) {
            type = getInstance().getTypeFactory().constructParametricType(parametrized, parametrized, parametersFor);
        } else {
            type = getInstance().getTypeFactory().constructParametricType(parametrized, parameterClasses);
        }
