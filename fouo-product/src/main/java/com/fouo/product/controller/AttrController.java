...
//		Attr detail = attrService.getOne(Condition.getQueryWrapper(attr));
     ...
     * 商品基本属性
     */
    @GetMapping("/base/listforspu")
    @ApiOperation(value = "分页", notes = "传入attr")
    public R<List<ProductAttrValue>> getSpuList(@RequestParam Long spuId) {
        return R.data(iProductAttrValueService.getSpuList(spuId));
    }
     ...
     * 根据spuId批量修改 商品属性
     */
    @PostMapping("/updateAttr/{spuId}")
    @ApiOperation(value = "批量修改", notes = "传入attr")
    public R updateAttr(@PathVariable("spuId") Long spuId,  @RequestBody List<ProductAttrValue> entities) {
        return R.status(iProductAttrValueService.updateSpuAttr(spuId,entities));
    }
    /**
     * 新增或修改 商品属性
     */
